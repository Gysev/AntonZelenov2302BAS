...
В пятом задании была реализована современная токен-базированная аутентификация с использованием JWT (JSON Web Token). Система использует два типа токенов: access токены (короткоживущие, 15 минут) для доступа к защищенным ресурсам и refresh токены (долгоживущие, 30 дней) для обновления пары токенов.

**Основные компоненты:**

1. **JwtTokenProvider** - генерация и валидация JWT токенов
   - Генерирует access и refresh токены с разными сроками жизни
   - Валидирует токены и извлекает информацию о пользователе

2. **JwtAuthenticationFilter** - фильтр Spring Security для проверки JWT токенов
   - Перехватывает запросы с заголовком `Authorization: Bearer <token>`
   - Валидирует access токены и устанавливает аутентификацию

3. **TokenService** - бизнес-логика работы с токенами
   - Генерирует пары токенов при логине
   - Обновляет токены через refresh endpoint
   - Управляет сессиями пользователей в БД

4. **UserSession** - entity для хранения сессий в БД
   - Отслеживает активные refresh токены
   - Статусы: ACTIVE, EXPIRED, REVOKED

**Endpoints:**

- `POST /api/auth/login` - получение пары токенов (access + refresh)
- `POST /api/auth/refresh` - обновление пары токенов (новая пара, старая сессия отзывается)

**Безопасность:**

- CSRF полностью отключен во всем проекте (токены в заголовках, не в cookies)
- Токены подписаны секретным ключом (HS512 алгоритм)
- Refresh Token Rotation - каждый refresh создает новую сессию, старая отзывается
- Старые refresh токены нельзя использовать повторно (статус REVOKED)

**Архитектура:**

- Две цепочки фильтров Spring Security:
  - JWT SecurityFilterChain (Order 1) - для JWT аутентификации
  - Basic Auth SecurityFilterChain (Order 2) - для Basic Auth (задания 1-4)
- Обе системы работают параллельно
- JWT фильтр имеет приоритет, Basic Auth - fallback

В результате была реализована современная, масштабируемая и безопасная система аутентификации, подходящая для REST API и мобильных приложений.

Задание 6. Создание цепочки сертификатов и настройка HTTPS

В шестом задании была реализована цепочка сертификатов и настроен HTTPS для Spring Boot приложения. Система использует иерархическую структуру сертификатов: Root CA → Intermediate CA → Server Certificate, где каждый сертификат содержит номер студенческого билета.

**Основные компоненты:**

1. **Цепочка сертификатов (3+ звена):**
   - Root CA (Корневой сертификат) - подписывает Intermediate CA
   - Intermediate CA (Промежуточный сертификат) - подписывает Server Certificate
   - Server Certificate (Серверный сертификат) - используется для HTTPS

2. **HTTPS конфигурация Spring Boot:**
   - Порт: 8443
   - Keystore: PKCS12 формат
   - SSL/TLS включен для всех endpoints

3. **CI/CD Pipeline:**
   - GitLab CI (`.gitlab-ci.yml`) или GitHub Actions (`.github/workflows/ci.yml`)
   - Безопасное хранение keystore в секретах (base64 encoded)
   - Автоматическая сборка, тестирование и упаковка артефактов

**Скрипты генерации:**

- `generate-certificates.ps1` - для Windows (PowerShell)
- `generate-certificates.sh` - для Linux/Mac (Bash)

**Безопасность:**

- Все приватные ключи и keystore исключены из git (`.gitignore`)
- Keystore хранится в CI/CD секретах (GitLab Variables / GitHub Secrets)
- Пароли передаются через переменные окружения
- Root CA импортируется в доверенные сертификаты системы

**Результат:**

- Приложение работает по HTTPS на порту 8443
- Браузер доверяет сертификату (зеленый замочек)
- CI/CD pipeline безопасно использует keystore для сборки
